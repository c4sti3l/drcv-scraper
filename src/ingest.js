...
class Ingest {
  ...
  _backfillLaps(nr, row, receivedAt) {
    ...
    // First time we see this driver in this run: nothing confirmed yet, but
    // the lasttime/secondlasttime/thirdlasttime window already covers their
    // first 1-3 laps, so treat it as "0 confirmed laps" instead of skipping -
    // otherwise short runs (e.g. a 2-lap Zeittraining) lose almost every lap.
    if (lastLaps == null) lastLaps = 0;
